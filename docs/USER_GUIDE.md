...
この文書は、`Distro/CYRUNE/public/free-v0.1/` に含まれる公開パッケージの使い方を、一般ユーザー向けに説明する文書です。  
この文書は current accepted public entry を置き換えません。製品の公開入口は `CYRUNE_Free_Public_Index.md` を参照してください。  
また、実行順序の canonical source は `GETTING_STARTED.md`、失敗時の remediation の canonical source は `TROUBLESHOOTING.md` です。

## 1. このパッケージでできること

CYRUNE Free v0.1 は、単一ユーザー向けの Control OS 公開パッケージです。  
...
この step では、公開パッケージ内の Free source tree から必要な binary を build し、`target/public-run/` 配下に実行用 state を作ります。  
