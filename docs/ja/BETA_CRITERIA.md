...
- public GitHub Actions workflow `public-ci`
- public documentation
- first-success runtime evidence
- 同一 beta line の Closed Gate closeout evidence
...
- Closed Gate closeout evidence は `main` 上の post-release evidence です。Closed Gate Report がこの repository 内で公開されない場合、それは release closeout evidence であり、public reader verification path の一部ではありません。
...
7. beta candidate に対する GitHub Actions workflow `public-ci` success
8. fresh `prepare-public-run.sh` -> `doctor.sh` -> `first-success.sh` verifier report
9. returned `evidence_id`、expected accepted-run evidence files、matching terminal binding marker、visible `working/working.json` raw hash
10. public docs consistency scan
11. license / third-party notice boundary check
12. 同一 beta line の Closed Gate closeout evidence。Closed Gate Report がこの repository 内で公開されない場合、それは release closeout evidence であり、public reader verification path の一部ではありません。
