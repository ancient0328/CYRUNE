...
`EVID-BOOTSTRAP` は、最初の ledger evidence directory が存在する前に使う working-source bootstrap marker です。これは ledger evidence directory name ではありません。accepted / rejected ledger evidence は引き続き `EVID-1` から始まります。

Citation evidence reference は `evidence_id` を必須とします。caller が chunk/span-level provenance を持つ場合は、`doc_id`、`chunk_id`、`span`、`quote_hash` も保持できます。空の granular field と invalid span は拒否されます。
...
public GitHub Actions workflow `public-ci` は次を確認します。
