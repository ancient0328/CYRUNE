...
この文書は、GitHub 公開リポジトリとしての CYRUNE Free v0.1 public alpha の engineering-facing structure と execution contract を説明する日本語 companion です。
...
この文書が説明するのは、公開される physical surface、public script の root 解決、`target/public-run/` に生成される state、public-run behavior を壊し得る変更点、maintenance 時の確認 predicate、そして public alpha が主張しない runtime / product scope です。
...
CYRUNE Free v0.1 public alpha では、次を採用します。
...
`docs/deferred/` は Free v0.1 alpha claim に自動採用されない future-publication / upper-tier material を含みます。
...
concrete carrier URL / filename / size / SHA256 は operational pins であり、product identity authority ではありません。
...
`scripts/first-success.sh` は、prepared `cyr` binary を通して no-LLM first-success request を実行します。
