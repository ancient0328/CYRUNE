...
この文書は authority surface や canonical source を置き換えません。公開入口と current accepted public truth は `CYRUNE_Free_Public_Index.md` を起点に確認してください。
...
4. どこを変えると shipped behavior が崩れるか
...
公開パッケージには、役割の異なる 3 つの surface があります。

1. discovery / authority surface  
   `README.md`、`docs/CYRUNE_Free_Public_Index.md`、`docs/GETTING_STARTED.md`、`docs/TROUBLESHOOTING.md`
2. supplementary package docs  
   `docs/` 配下の補助文書群
3. shipped runnable source tree  
   `free/v0.1/0/`
...
2. 何が生成されれば prepare が成立したと言えるか
3. `doctor` と `first-success` は何を stdout に返すか
4. どの key / path / mode が shipped behavior の invariant か
...
`docs/` には、製品説明、public index、補助文書、diagram files が含まれます。  
`scripts/` には、公開ユーザー向けの 3 本の shell script が含まれます。  
`free/v0.1/0/` には shipped runnable source tree が含まれます。

`free/v0.1/0/` のうち、shipped behavior を理解するうえで最低限見るべき実装 family は次です。
...
4. `crates/cyrune-core-contract/`  
   request / result / denial / ID family を持つ closed contract surface
...
2. stdout は空文字列である
3. exit code は `0` である
4. `"$STATE_ROOT/bin/cyr"` が存在する
5. `"$STATE_ROOT/bin/cyrune-daemon"` が存在する
6. `"$STATE_ROOT/home/"` が存在する
...
- shell は `bash`
- build host に `cargo` と `install` が存在する
...
次を変えると shipped behavior に直結します。
...
2. `docs/CYRUNE-Free_Canonical.md`
3. `docs/CYRUNE.md`
