...
This is the public beta first-success path for the `v0.1.1-beta.1` release contract. It is expected to prepare local state from the pinned beta carrier, pass `doctor`, and return a verifier report with `verified: true` and `outcome: "accepted"` for the packaged Free v0.1 no-LLM path. It does not prove production maturity, native distribution, OS-level sandbox isolation, enforcement-complete classification / MAC, or capabilities outside the explicit Free v0.1 public beta scope.
