...
この段落は当時の execution history であり、current GitHub publication model の authority ではない。2026-04-26 JST 時点の current public model は root repository `ancient0328/CYRUNE`、latest surface `main`、immutable snapshot tag `v0.1.0`、existing compatibility tag `v0.1`、no `v0.1` branch である。
同日、`publish_release_package_to_github.py` の actual execution により then-current repository root tracked set `README.md` / `docs/`、then-current release tag `v0.1`、then-current release title `CYRUNE Free v0.1`、exact asset URL `cyrune-free-v0.1.tar.gz`、third-party reachability が materialize され、`SGM-D` は完了した。
...
同日、separate future scope `GPC` が fixed problem として追加され、`free/v0.1/0` stand-alone complete root、then-current `public/free-v0.1/free/v0.1/0` GitHub publication branch、tracked public branch surface と GitHub-hosted non-tracked carrier の exact boundary を再固定する ADR / implementation roadmap / test roadmap が authority として追加された。
...
3. source-side publication unit root は `Distro/CYRUNE/public/free-v0.1/` のみ。この path は private workspace 側の staging root であり、GitHub public reader の local path ではない
...
fixed problem `GPC` は、`free/v0.1/0` を stand-alone complete root、then-current `public/free-v0.1/free/v0.1/0` を GitHub publication branch として再固定した separate future scope である。current public repository model は root `main` + immutable `v0.1.0` tag である。
